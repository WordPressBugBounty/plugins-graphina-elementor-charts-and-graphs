 ...
 * RadarChart Class
 *
 * This class defines the Radar Chart widget for the Graphina plugin, integrating
 * with Elementor to provide a customizable Radar Chart based on ApexCharts.
 *
 * Product: Graphina - Elementor Charts and Graphs
 * Purpose: To allow users to create dynamic and interactive Radar Charts.
 */
class RadarChart extends GraphinaApexChartBase {
	...
	public function get_title() {
		return __( 'Radar Chart', 'graphina-charts-for-elementor' );
	}
	 ...
	 * Get Chart Type
	 *
	 * Defines the type of chart this widget represents. In this case, 'radar'.
	 *
	...
	protected function graphina_prepare_chart_common_options($settings, $chart_type, $element_id)
	{
		...
		$marker_size = $marker_stroke_color = $stroke_widths = $strock_dash_array = $categories = $marker_size = $gradient_new = $strokeDashArray  = $nested_temp = $tooltip_series = $marker_stoke_width = $marker_shape = $second_gradient = $fill_pattern = $gradient = $yaxis = array();
		...
		$chart_options['plotOptions'] = array(
			'radar' => array(
				'polygons' => array(
					'strokeColors'    => $settings[GRAPHINA_PREFIX . $chart_type . '_chart_plot_stroke_color'],
					'connectorColors' => $settings[GRAPHINA_PREFIX . $chart_type . '_chart_plot_stroke_color'],
					'fill'            => array(
						'colors' => array($settings[GRAPHINA_PREFIX . $chart_type . '_chart_plot_color']),
					),
				),
			),
		);
		$chart_options['dataLabels']['offsetY']      = $settings[GRAPHINA_PREFIX . $chart_type . '_chart_datalabel_offsety'];
		$chart_options['dataLabels']['offsetX']      = $settings[GRAPHINA_PREFIX . $chart_type . '_chart_datalabel_offsetx'];
		$chart_options['stroke']['width']            = intval($settings[GRAPHINA_PREFIX . $chart_type . '_chart_stroke_size']);
		...
		$chart_options['yaxis']['show'] = ! empty($settings[GRAPHINA_PREFIX . $chart_type . '_chart_yaxis_datalabel_show']) && $settings[GRAPHINA_PREFIX . $chart_type . '_chart_yaxis_datalabel_show'] === 'yes' ? true : false;
